...
export const fetchBanner = async (pageNumber = 0, pageSize = 10, headers) => {
    return await axios({
        method: 'get',
        url: `${BaseUrl}/advertisement/v1/getAllAdvertisementByPagination/${pageNumber}/${pageSize}?pageNumber=${pageNumber}&pageSize=${pageSize}`,
        headers: headers
    });
};
...
export const getAdvertiseById = async (id, headers) => {
    return await axios({
        method: 'GET',
        url: `${BaseUrl}/advertisement/v1/getAdvertisementByAdvertisementId/${id}`,
        headers: headers
    });
};
