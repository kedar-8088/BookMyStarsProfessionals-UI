...
export const addFilesToShowcase = async (showcaseId, files, metadata = {}) => {
  try {
    ...
    files.forEach((file, index) => {
      ...
      formData.append('files', file); // Use 'files' (plural) as the backend might expect this
      formData.append('file', file); // Also try 'file' (singular) for compatibility
      
      // Add file metadata - ensure all values are defined before calling toString()
      formData.append(`file_${index}_name`, file.name || '');
      formData.append(`file_${index}_type`, file.type || '');
      formData.append(`file_${index}_size`, (file.size || 0).toString());
      formData.append(`file_${index}_isVideo`, String(file.type?.startsWith('video/') || false));
      formData.append(`file_${index}_isImage`, String(file.type?.startsWith('image/') || false));
      formData.append(`file_${index}_isPrimary`, String((metadata.isPrimary === true && index === 0) || false));
      formData.append(`file_${index}_displayOrder`, String(metadata.displayOrder || index + 1));
      formData.append(`file_${index}_description`, metadata.description || `Portfolio ${file.type?.startsWith('video/') ? 'video' : 'image'} file`);
    });
    
    formData.append('fileCount', files.length.toString());
    
    console.log(`Attempting to add ${files.length} file(s) to showcase ${showcaseId}`);
    ...
    console.log('addFilesToShowcase response:', response.data);
    return response.data;
  } catch (error) {
    console.error('addFilesToShowcase error:', error);
    // If the specific endpoint doesn't exist, return a failure response
    if (error.response?.status === 404 || error.response?.status === 405) {
      console.warn('Showcase file upload endpoint not available (404/405)');
